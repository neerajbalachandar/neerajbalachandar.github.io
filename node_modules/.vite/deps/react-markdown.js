...
import {
  ...
  unified,
  visit
} from "./chunk-ZWUQEVVX.js";
import "./chunk-C2YVY7QT.js";
import {
  require_react
  ...
  function documentContinue(code2) {
    ...
    if (self.containerState._closeFlow) {
      ...
      while (index < self.events.length) {
        self.events[index][1].end = Object.assign({}, point3);
        index++;
      }
      splice(
        self.events,
...
function compiler(options) {
  ...
  function enter(node2, token, errorHandler) {
    ...
    node2.position = { start: point(token.start) };
    return node2;
  }
  function closer(and) {
    return close;
...
function all2(state, parent) {
  ...
  if ("children" in parent) {
    ...
    while (++index < nodes.length) {
      const result = one2(state, nodes[index], parent);
      if (result) {
...
function rehypeFilter(options) {
  ...
  if (options.allowedElements || options.disallowedElements || options.allowElement) {
    return (tree) => {
      visit(tree, "element", (node2, index, parent_) => {
        const parent = (
          /** @type {Element|Root} */
          parent_
        );
...
function toReact(context, node2, index, parent) {
  ...
  if (node2.properties) {
    for (property in node2.properties) {
      if (own4.call(node2.properties, property)) {
  ...
  const component = options.components && own4.call(options.components, name) ? options.components[name] : name;
...
function getElementsBeforeCount(parent, node2) {
  ...
  while (++index < parent.children.length) {
    if (parent.children[index] === node2) break;
...
function addProperty(props, prop, value, ctx) {
  ...
  if (info.space && info.property) {
    props[own4.call(hastToReact, info.property) ? hastToReact[info.property] : info.property] = result;
...
function ReactMarkdown(options) {
  for (const key in deprecated) {
    if (own5.call(deprecated, key) && own5.call(options, key)) {
