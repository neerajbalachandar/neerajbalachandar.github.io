...
A sequence of nested sets $D_1 \subset D_2 \subset \dots \subset D$ such that for any $\epsilon > 0$, there exists an $L$ where $D \subseteq \bigcup_{x \in D_L} \{y : \|y - x\|_2 < \epsilon\}$. Any member $D_L$ is called a discretization of $D$.

**Discretized Uniform Risk**
Measures the worst-case error over a compact set of input functions $K \subset \mathcal{A}$ when using the discrete approximation:
$$R_K(\mathcal{G}, \hat{\mathcal{G}}, D_L) = \sup_{a \in K} \|\hat{\mathcal{G}}(D_L, a|_{D_L}) - \mathcal{G}(a)\|_{\mathcal{U}}$$
*   $\mathcal{G}: \mathcal{A} \to \mathcal{U}$: True continuous operator.
*   $\hat{\mathcal{G}}: \mathbb{R}^{Ld} \times \mathbb{R}^{Lm} \to \mathcal{U}$: Discrete approximation map (takes discretized input, outputs a continuous function in $\mathcal{U}$).
*   $a|_{D_L}$: Point-wise evaluations of the input function.

**Discretization Invariance**
Given a finite-dimensional parameter space $\Theta \subseteq \mathbb{R}^p$ and a parametric operator $\mathcal{G}: \mathcal{A} \times \Theta \to \mathcal{U}$, the architecture is discretization-invariant if there exists a sequence of discrete maps $\hat{\mathcal{G}}_L : \mathbb{R}^{Ld} \times \mathbb{R}^{Lm} \times \Theta \to \mathcal{U}$ such that for any fixed parameters $\theta \in \Theta$ and any compact set $K \subset \mathcal{A}$:
$$\lim_{L \to \infty} R_K(\mathcal{G}(\cdot, \theta), \hat{\mathcal{G}}_L(\cdot, \cdot, \theta), D_L) = 0$$
*   Translation: As the discretization is refined ($L \to \infty$), the discrete approximation converges uniformly to the true continuous operator.
